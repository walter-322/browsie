...
class XPCShellTestThread(Thread):
    def __init__(
        ...
        **kwargs,
        ...
        self.rootPrefsDir = kwargs.get("rootPrefsDir")
    ...
    def updateTestPrefsFile(self):
        ...
        if "prefs" in self.test_object:
            ...
            # rootPrefsDir contains a user.js file, generated by buildPrefsFile
            profile.merge(self.rootPrefsDir, interpolation=interpolation)
...
class XPCShellTests:
    ...
    def buildPrefsFile(self, extraPrefs):
        ...
        # Concurrent harness runs can share self.tempDir, so keep their root
        # preferences in per-run directories.
        os.makedirs(self.tempDir, exist_ok=True)
        self.rootPrefsDir = mkdtemp(prefix="xpc-prefs-", dir=self.tempDir)
        profile = Profile(profile=self.rootPrefsDir, restore=False)
    ...
    def runTests(self, options, testClass=XPCShellTestThread, mobileArgs=None):
        ...
        self.cleanup_dir_list = [self.rootPrefsDir]
        ...
        kwargs = {
            ...
            "rootPrefsDir": self.rootPrefsDir,
